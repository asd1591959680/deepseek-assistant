...
const DEFAULT_SYSTEM_PROMPT = `你是一个专业的知识库助手。请基于以下检索到的上下文内容来回答用户的问题。
...
export const useRagStore = defineStore("rag", () => {
  ...
  async function addDocument(file: File) {
    ...
    try {
      // // 初始化嵌入模型
      processingMessage.value = "正在初始化嵌入模型...";
      await initPipeline();

      // 提取文本
      processingMessage.value = `正在解析文档：${file.name}`;
      ...
      doc.status = "error";
      doc.errorMsg = (e as Error).message;
    } finally {
